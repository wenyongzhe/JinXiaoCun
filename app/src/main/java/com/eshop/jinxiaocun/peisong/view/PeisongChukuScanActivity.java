...
import com.eshop.jinxiaocun.peisong.adapter.PeisongChukuScanAdapter;
...
public class PeisongChukuScanActivity extends CommonBaseScanActivity implements INetWorResult {
    ...
    private String mStr_OrderNo;//配送出库单据号
    ...
    protected void initView() {
        ...
        mAdapter = new PeisongChukuScanAdapter(mListDatas);
        ...
        public boolean onKey(View v, int keyCode, KeyEvent event) {
            if(keyCode == KeyEvent.KEYCODE_ENTER && event.getAction()== KeyEvent.ACTION_UP){
                if(TextUtils.isEmpty(mTvTiaoRu.getText().toString().trim())){
                    AlertUtil.showToast("请选择调入，再添加商品!");
                    return false;
                }
    ...
    private void uploadRecordHeadData(){
        ...
        bean.JsonData.SheetType = Config.YwType.MO.toString(); //单据类型
    ...
    private void getOrderDetail(DanJuMainBeanResultItem selectMainBean) {
        if (selectMainBean != null) {
            ...
            mTvTiaoRu.setText(selectMainBean.getYHShopName());
    ...
    public void handleResule(int flag, Object o) {
        switch (flag){
            ...
            case Config.MESSAGE_RESULT_SUCCESS:
                mOtherApi.sheetSave(Config.YwType.MO.toString(),mStr_OrderNo);//保存业务单据
                break;
            //保存业务单据 成功
    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        if(requestCode == 2 && resultCode == 22){
            ...
            mTvTiaoRu.setText(mStoreInfo.getName());
...
//        在上传保存前，获取单据号
        if(TextUtils.isEmpty(mStr_OrderNo)){
            ...
            bean.JsonData.trans_no = Config.YwType.MO.toString();
