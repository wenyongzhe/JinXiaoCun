...
public class PifaTuihuoScanActivity extends CommonBaseScanActivity implements INetWorResult {
    ...
    private PifaTuihuoScanAdapter mAdapter;
    ...
    private final String mSheetType = "本地_"+Config.YwType.RI.toString();
    private boolean isCiteOrder;//false不是引单  true为引单

    @Override
    protected int getLayoutContentId() {
        return R.layout.activity_pifa_tuihuo_scan;
    }
    ...
    protected void initView() {
        ...
        setTopToolBar("批发退货生成", R.mipmap.ic_left_light, "", 0, "");
        ...
        mTvUser.setDrawableRightClick(new DrawableTextView.DrawableRightClickListener() {
            ...
            public void onDrawableRightClickListener(View view) {
                ...
                Intent intent = new Intent(PifaTuihuoScanActivity.this, SelectCustomerListActivity.class);
        ...
        mAdapter = new PifaTuihuoScanAdapter(mListDatas);
    ...
    private void getOrderDetail(DanJuMainBeanResultItem selectMainBean) {
        if (selectMainBean != null) {
            ...
            String newOrderNo = BusinessBLL.getInstance().
                    getNewBillNO("PifaTuihuo",3,true);
...
//        }
    ...
    private void addGoodsData(GetClassPluResult scanOrSelectGoods){
        if(scanOrSelectGoods !=null){
            ...
            if(!isSame){//不存在添加 ，已经存在直接刷新
                ...
                mOtherApi.getOrderGoodsPrice(Config.YwType.RI.toString(),"",scanOrSelectGoods.getItem_no(),SupCust_No);
                return;
            }
    ...
    private void uploadRecordHeadData(){
        ...
        bean.JsonData.SheetType = Config.YwType.RI.toString(); //单据类型
        ...
        switch (flag){
            ...
            case Config.MESSAGE_OK:
                ...
                for (OrderDetailBeanResult detailData : orderDetailDatas) {
                    ...
                    obj.setSale_qnty(detailData.getCheckNum()+"");//实际数量
                    ...
                    if(!isCiteOrder) {//不是引单时  从列表进入的网上取到的主表取其明细
                        ...
                        old_obj.setSale_qnty(detailData.getCheckNum()+"");//实际数量
            ...
            case Config.MESSAGE_GOODS_INFOR:
                ...
                if(goodsData !=null && goodsData.size()>0){
                    if(goodsData.size()==1){
                        ...
                        //多条数据 弹出选择其中一条
                        Intent intent = new Intent(PifaTuihuoScanActivity.this,SelectPandianGoodsListActivity.class);
            ...
            case Config.MESSAGE_RESULT_SUCCESS:
                mOtherApi.sheetSave(Config.YwType.RI.toString(),mStr_OrderNo);//保存业务单据
                break;
            //保存业务单据 成功
    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        if(requestCode == 4 && resultCode == RESULT_OK){
            ...
            selectMainBean.setSheetType(Config.YwType.RI.toString());
...
//        在上传保存前，获取批发订单单据号
        if(TextUtils.isEmpty(mStr_OrderNo)){
            ...
            bean.JsonData.trans_no = Config.YwType.RI.toString();
    ...
    private class GetDBDatas extends AsyncTask<String,String,String> {
        // 弱引用是允许被gc回收的;
        private final WeakReference<PifaTuihuoScanActivity> weakActivity;
        GetDBDatas(PifaTuihuoScanActivity activity) {
        ...
        protected void onProgressUpdate(String... values) {
            ...
            PifaTuihuoScanActivity activity = weakActivity.get();
            if (activity == null
