...
/**
 * <p>This listener class informs the client when the login_user sets
 * a date and time by pressing "OK", or cancels the dialog.</p>
 *
 * <p>Overriding {@code onDateTimeCancel()} is optional. The client
 * can override this to listen for when the login_user cancels the dialog.
 * This is called when the login_user presses the dialog's Cancel button,
...
{
    /**
     * Informs the client when the login_user presses "OK"
     * and selects a date and time.
     *
     * @param date  The {@code Date} object that contains the date
     *              and time that the login_user has selected.
     */
    public abstract void onDateTimeSet(Date date);

    /**
     * Informs the client when the login_user cancels the
